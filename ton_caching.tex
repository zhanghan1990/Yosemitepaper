...
Some theory papers such as \cite{qiu2015minimizing} set weight to coflow to minimize average weight coflow completion time. 
We think this is good direction as they take this difference into consideration.
However, methods such as  \cite{qiu2015minimizing} will solve complex equations which make them too complex to use in practise.
...
In this part, we define Weight Coflow Completion Optimization (WCCO) problem and prove its equivalent to
the weight job completion optimization in concurrent open shop\cite{roemer2006note}.
...
However, in reality, applications just have different importance and they do not have to finish before deadlines,
so that we abandon deadline to present the importance of applications.
In this paper, we use weight to express the importance of applicatinos.
...
In this case, instead of minimizing average coflow completion time, we try to minimize average weight coflow completion time.
...
Where minimizing their average weight coflow completion time is equivalent to 
minimizing their sum of weight coflow completion time, just as (\ref{eq:WCCO-SC}) shown. 
...
In this section, we prove that the target of minimizing average weight coflow completion time
is equivalent to the problem minimizing average weight job completion time in concurrent open shop problem.
\begin{proposition} \label{WCCO-eq}
Weight Coflow Completion Optimization (WCCO) problem is equivalent to the problem of minimizing the sum of weight job completion time in a concurrent open shop.
\end{proposition}
...
\end{IEEEproof}
It has been proved that minimizing average weight job completion time is a NP-hard problem \cite{mastrolilli2010minimizing}\cite{chen2000supply}\cite{roemer2006note}\cite{qiu2016experimental},
...
Currently, the best known results for minimizing average weight job completion time is a 2-approximation greedy algorithm\cite{kumar2011lp}\cite{mastrolilli2010minimizing}.
According to the correspondence between WCCO and minimizing average weight job completion time problem that is introduced at section \ref{np-hard-proof}, 
...
Firstly, we consider the schedule of 100 coflows served by a small-scale cluster consisting of 60 hosts. 
...
More details of the simulator is shown at Section \ref{evaluation}. 
Let all coflows arrive at time 0 and set weight within 1 to 5.
...
Table \ref{tab:loss} shows the proportions of $\frac{avg \;wcct\  of \ online}{avg\; wcct \ of \  2-appr}-1$, where $avg\;wcct $ means average weight coflow completion time.
...
In this section, we introduce the design of Yosemite which is a system aims to minimize average weight coflow completion time. 
Yosemite includes about 6000 lines of java as well as scala code and similar to Alluxio\cite{alluxio}, hadoop\cite{hadoop}, its design follows the actor model\cite{actor}.
...
\item  Running file distribute application at our private cloud platform, we find Yosemite improves 30\% over Varys on minimizing average weight coflow completion time.
...
\textbf{Metrics}. In our experiment,  metrics for comparison is the improvement in average weight coflow completion time of jobs (when its last task finished) in the workload. 
...
\caption{[Simulation] Detail of WCCT for Yosemite, Barrat, Varys,  Aalo, TCP fair-sharing under real data center traffic trace,TCP fair-sharing is selected as the baseline.}
\vspace{-0.1 in}
...
\caption{[Simulation] Detail of CCT for Yosemite, Barrat, Varys,  Aalo, TCP fair-sharing under real data center traffic trace, TCP fair-sharing is selected as the baseline.}
...
\caption{[Simulation] Yosemite performance comparison with Baraat and Varys, Aalo under different settings, TCP fair-sharing is selected as the baseline.}
...
Note TCP fair-sharing is selected as the baseline in this group.
 ...
 Fig. \ref{evalution_real_weight_fig}(a) implies that Yosemite greatly reduces the average weight coflow completion time across all coflow types. 
...
the 95th factor of improvement.
 For the 95th case, we can find that Yosemite has the improvement factors on average weight coflow completion time  of 30 (Narrow\&Short),20 (Narrow\&Long), 2.5 (Wide\&Short), 1.5  (Wide\&Long), and 2.5 (ALL), however Varys has 22 (Narrow\&Short),14 (Narrow\&Long), 2.1 (Wide\&Short), 1.2  (Wide\&Long) and 2.1 (ALL) and other 
 ...
 That is to say, for applications which involves lot long flows, weight completion time will have long duration using Yosemite and this is trivial for private cloud.
...
The reason for this is that, when fraction of thin coflows is small, conflict probability between coflows is large, so it is necessary to decide a suitable scheduling sequence of coflows.
 ...
 Weight setting has huge influence on the performance of weight coflow completion time. 
...
It is hard to  find the optimal schedule is infeasible, we tried to find an LP-based solution \cite{qiu2015minimizing}. 
We run the experiment with facebook trace and generate weight within 1 to 5. 
Repeat the experiment 20 times and the result is shown as Fig. \ref{LP-based-fig}. 
We can see that Yosemite and the LP-based solution has less than 10\% performance gap.
...
We design and implement Yosemite-a coflow scheduling system that aims to minimize average weight coflow completion time.
We test the performance of it by trace-driven simulation and test-bed deployment.
Experiments show Yosemite can reduce average weight coflow completion time significantly.  

Although Yosemite works well on minimizing average weight coflow completion time, it is Clairvoyant method.

...
As a result, the average flow completion time (FCT) tends to be reduced, so does applications' transport lantency. 
...
Repeat the process 100 times and Fig .\ref{evalution_real_weight_fig} shows the performance comparison between Yosemite, Barrat, Varys,  Aalo and TCP fair-sharing. 
Note TCP fair-sharing is selected as the baseline in this group.


 Fig. \ref{evalution_real_weight_fig}(a) implies that Yosemite greatly reduces the average wcct across all coflow types. 
 The improvement factors of Yosemite are 3.5 (Narrow\&Short), 1.6 (Narrow\&Long), 1.7 (Wide\&Short), 1.4  (Wide\&Long) and 1.5 (ALL), 
 while that of Varys are 2.4 (Narrow\&Short), 0.9 (Narrow\&Long), 1.2 (Wide\&Short), 1.3 (Wide\&Long) and 1.2 (ALL). 
...
the 95th factor of improvement.
 ...
 One may be interested in the coflow completion time (cct) performance of Yosemite. 
...
From Algorithm \ref{online-algorithm},  we can see performance of Yosemite is mainly influenced by coflow length, coflow width, coflow weight and coflow concurrency. 
 ...
 \begin{table}[!htb]
          ...
          \begin{tabulary}{\textwidth}{cccccr}
              ...
              \midrule
              \multicolumn{1}{r}{512}&[100KB,10GB]   &60& [1,10]  & [0s,1000s]&1GB/s\\
              \bottomrule
          \end{tabulary}
      \end{table}
...
We repeat each group of experiments 100 times and draw maximum, minimum as well as average value in the picture.
 ...
 For example, when K is 200MB,  improvement of Yosemite is 2 and when K is 700MB, improvement of Yosemite is 3. 
...
We run the experiment with facebook trace and generate weight within [1,5]. 
 Repeat the experiment 20 times and experiment is shown as Fig. \ref{LP-based-fig}. 
 We can see that in total Yosemite has less than 10\% performance loss. 
...
Although Yosemite works well on minimizing average weight coflow completion time, It is Clairvoyant method and this constraints its usage.
Indeed in data center network, applications such as file backup, file distributes can know these information.
For some computation applications, they are hard to get.
